...
public class OrdenComprasLifecycle {
    ...
    public void init(@Observes StartupEvent evt, Vertx vertx) throws UnknownHostException {
        ...
        String httpCheckUrl = String.format("http://%s:%d/ordenComp", ipAddress.getHostAddress(), port);

        client.registerServiceAndAwait(
                new ServiceOptions()
                        .setName("app-ordenComp")
                        .setId(serviceId)
                        ...
                        .setTags(
                                List.of(
                                        "traefik.enable=true",
                                        "traefik.http.routers.app-books.rule=PathPrefix(`/app-ordenComp`)",
                                        "traefik.http.routers.app-books.middlewares=app-ordenComp",
                                        "traefik.http.middlewares.app-books.stripPrefix.prefixes=/app-ordenComp")
                        )
                        .setCheckOptions(
                                new CheckOptions()
