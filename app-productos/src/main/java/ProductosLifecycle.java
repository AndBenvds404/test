...
public class ProductosLifecycle {
    ...
    public void init(@Observes StartupEvent evt, Vertx vertx) throws UnknownHostException {
        ...
        String httpCheckUrl = String.format("http://%s:%d/productos", ipAddress.getHostAddress(), port);
        ...
        client.registerServiceAndAwait(
                new ServiceOptions()
                        ...
                        .setTags(
                                List.of(
                                        "traefik.enable=true",
                                        "traefik.http.routers.app-books.rule=PathPrefix(`/app-productos`)",
                                        "traefik.http.routers.app-books.middlewares=app-productos",
                                        "traefik.http.middlewares.app-books.stripPrefix.prefixes=/app-productos")
                        )
                        .setCheckOptions(
                                new CheckOptions()
